...
func GitDiff(a, b interface{}) string {
...
func (d *Diff) diff(aVal, bVal reflect.Value, path Path) bool {
	...
	case reflect.Map, reflect.Ptr, reflect.Func, reflect.Chan, reflect.Slice:
		...
		if aVal.IsNil() || bVal.IsNil() {
			d.Modified[&localPath] = bVal.Interface()
			if bVal.IsNil() {
				d.addLine('+', localPath, bVal.Interface())
			} else {
				d.addLine('-', localPath, aVal.Interface())
			}
			return false
		}
	}
	...
	case reflect.Array, reflect.Slice:
		...
		if aLen > bLen {
			for i := bLen; i < aLen; i++ {
				...
				d.addLine('-', localPath, aVal.Index(i).Interface())
				equal = false
			}
		} else if aLen < bLen {
			for i := aLen; i < bLen; i++ {
				...
				d.addLine('+', localPath, bVal.Index(i).Interface())
				equal = false
			}
		}
	case reflect.Map:
		for _, key := range aVal.MapKeys() {
			...
			if !bI.IsValid() {
				...
				d.addLine('-', localPath, aI.Interface())
				equal = false
		...
		for _, key := range bVal.MapKeys() {
			...
			if !aI.IsValid() {
				...
				d.addLine('+', localPath, bI.Interface())
				equal = false
			}
		}
	case reflect.Struct:
...
func (d *Diff) addLine(pref byte, localPath Path, val interface{}) {
	d.Lines = append(d.Lines, string(pref)+strings.TrimPrefix(localPath[len(localPath)-1].String(), ".")+": "+printTab(len(localPath)-1)+fmt.Sprint(val))
}
